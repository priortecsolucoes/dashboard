...
class AmgDash:
    def __init__(self):
        ...
        try:
            self.engineAMG = create_engine(db_url)

            # Testa a conexão
            with self.engineAMG.connect() as conn:
                print("Conexão com o banco AMG estabelecida com sucesso!")

        except Exception as e:
            print(f"Erro ao conectar com o banco AMG: {e}")
            self.engineAMG = None
    ...
    def getErrors(self):
        ...
        try:
            ...
            query = """
                ...
                ORDER BY total DESC;
            """
            df = pd.read_sql_query(query, self.engineAMG)
            if df.empty:
    ...
    def getFileById(self, file_id):
        try:
            query = """
                ...
                WHERE "ID" = %s
            """
            df = pd.read_sql_query(query, self.engineAMG, params=(file_id,))
            return df
        except Exception as e:
    ...
    def get_aso_reading_status(self):
        try:
            ...
            tempo_df = pd.read_sql_query(tempo_query, self.engineAMG)
